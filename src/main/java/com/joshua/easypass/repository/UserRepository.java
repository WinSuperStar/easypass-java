...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM User u WHERE u.username like %:username% AND u.phone like %:phone% AND u.rolename like %:role% AND u.state like %:state%  AND   u.userid = :userid ")
    public User[] getUsersByOwner(@Param("username") String username, @Param("phone") String phone, @Param("role") String role, @Param("state") String state,@Param("userid") Integer userid);
