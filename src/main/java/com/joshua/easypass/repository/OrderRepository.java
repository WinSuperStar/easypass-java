...
public interface OrderRepository extends JpaRepository<Order, Integer>, JpaSpecificationExecutor<Order> {
    ...
    public void updateOdr(@Param("orderid") Integer orderid, @Param("state") String state);

    @Query("SELECT v FROM Order v WHERE v.orderid = :orderid")
    public Order getOdr(@Param("orderid") Integer orderid);

    @Modifying
    @Query(value="DELETE FROM order_ WHERE orderid = :orderid", nativeQuery = true)
    public void delOdr(@Param("orderid") Integer orderid);
